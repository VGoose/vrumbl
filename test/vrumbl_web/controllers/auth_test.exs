defmodule VrumblWeb.AuthTest do
  use VrumblWeb.ConnCase, async: true
  alias VrumblWeb.Auth

  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(VrumblWeb.Router, :browser)
      |> get("/")
  ...
  test "authenticate_user for existing current_user", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %Vrumbl.Accounts.User{})
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%Vrumbl.Accounts.User{id: 123})
      |> send_resp(:ok, "")
