defmodule VrumblWeb.VideoControllerTest do
  use VrumblWeb.ConnCase, async: true

  test "requires user authentication on all calls", %{conn: conn} do
    Enum.each(
      [
  ...
  describe "logged in tests" do
    ...
    test "list all user's videos on index", %{conn: conn, user: user} do
      user_video = video_fixture(user, title: "my video")
      other_video = video_fixture(user_fixture(username: "foo"), title: "random video")
